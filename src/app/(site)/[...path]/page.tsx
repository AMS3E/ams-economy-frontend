...
export default async function LandingPage({ params, searchParams }: PageParams) {
  ...
  return (
    <>
      ...
      {feed.section && <SectionHead section={feed.section} matika={feed.matika} basePath={`/${path.join("/")}`} />}
      {feed.topic && <TopicHead topic={feed.topic} matika={feed.matika} />}

      <LandingTail tail={feed.tail} />
    </>
  );
}
