...
| Editor save (title, description, release date, schedule, video source, artwork) | `POST wp/v2/movie\|tv_show/{id}` | W | ✅ | Browser-verified on a throwaway draft (round-tripped via API). Description writes `excerpt` (post_excerpt = the public page's text) since 2026-08-27 — `content` is the old WP page's layout canvas, never written. Video source is movies-only; status/`_seasons` out of scope |
